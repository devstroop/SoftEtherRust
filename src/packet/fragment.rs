...
impl FragmentReassembler {
    ...
    pub fn process(&mut self, ip_packet: &[u8]) -> Option<Vec<u8>> {
        ...
        let state = self.states.entry(key.clone()).or_insert_with(ReassemblyState::new);
    ...
    pub fn cleanup(&mut self) {
        ...
        self.states.retain(|_, state| {
            now.duration_since(state.first_seen) < self.timeout
        });
        ...
        match fragment_ipv4_packet(&packet, 576) {
            FragmentResult::Fragmented(fragments) => {
                ...
                for (i, frag) in fragments.iter().enumerate() {
                    ...
                    assert_eq!(total_len, frag.len(), "Fragment {} total length mismatch", i);
                ...
                assert_eq!(flags & IP_FLAG_MF, 0, "Last fragment should not have MF flag");
                ...
                for frag in fragments.iter().take(fragments.len() - 1) {
                    ...
                    assert_ne!(flags & IP_FLAG_MF, 0, "Non-last fragment should have MF flag");
                }
            }
