...
impl L2L3Translator {
    ...
    pub fn set_gateway_mac(&mut self, gateway_mac: [u8; 6]) {
        debug!(
            "Setting gateway MAC: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
            gateway_mac[0],
            gateway_mac[1],
            gateway_mac[2],
            gateway_mac[3],
    ...
    /// Set our IP address (from DHCP or static configuration)
    /// This allows the translator to respond to ARP probes from the DHCP server
    pub fn set_our_ip(&mut self, our_ip: Ipv4Addr) {
        debug!("Setting our IP: {}", our_ip);
                ...
                if self.gateway_mac.is_none() && payload.len() >= 20 {
                    ...
                    if let Some(our_ip) = self.our_ip {
                        if src_ip != our_ip && !src_ip.is_link_local() {
                            ...
                            {
                                let src_mac: [u8; 6] =
                                    eth_frame[6..12].try_into().unwrap_or([0; 6]);
                                self.gateway_mac = Some(src_mac);
            ...
            1 => {
                ...
                if let Some(our_ip) = self.our_ip {
                    if target_ip == our_ip {
                        let reply = self
                            .arp_handler
                            .build_arp_reply(our_ip, sender_mac, sender_ip)?;
