    ...
    pub fn set_adapter_tx(&self, mut rx: mpsc::UnboundedReceiver<Vec<u8>>) {
        ...
        let task = tokio::spawn(async move {
            while let Some(frame) = rx.recv().await {
                debug!(
                    "📥 Dataplane received frame from adapter: {} bytes",
                    frame.len()
                );
                ...
                if let Some(id) = target_id {
                    ...
                    warn!(
                        "dataplane: no active links for adapter TX frame (dropping {} bytes)",
                        frame.len()
                    );
                }
            }
    ...
    pub fn register_link(&self, tls: TlsStream<TcpStream>, direction: i32) -> u64 {
        ...
        let rx_handle = tokio::task::spawn_blocking(move || {
            ...
            loop {
                ...
                if !frames.is_empty() {
                    debug!(
                        "📨 Link RX: received {} frame(s) from VPN server",
                        frames.len()
                    );
                ...
                for f in &frames {
                    if f.len() >= 42 {
                        ...
                        if ethertype == 0x0806 {
                            // ARP packet
                            info!("📬 Link RX: ARP packet received ({} bytes)", f.len());
                            ...
                            if f.len() > ip_proto_offset {
                                ...
                                if ip_proto == 17 {
                                    ...
                                    if f.len() >= udp_offset + 4 {
                                        let src_port =
                                            u16::from_be_bytes([f[udp_offset], f[udp_offset + 1]]);
                ...
                if let Some(ext) = adapter_tx_opt {
                    debug!(
                        "📨 Link RX: forwarding {} frame(s) to adapter",
                        frames.len()
                    );
                    for f in &frames {
                    ...
                    if !frames.is_empty() {
                        warn!(
                            "📨 Link RX: adapter channel not set, dropping {} frame(s)",
                            frames.len()
                        );
                    }
                }
        ...
        let tx_handle = tokio::spawn(async move {
            while let Some(frame) = writer_rx.recv().await {
                let len = frame.len();
                debug!(
                    "📤 Link writer: received frame from channel ({} bytes), writing to TLS",
                    len
                );
