// Publicly exported digital watermark bytes.
// Must match original C (Cedar/WaterMark.c) and Go implementation exactly.
// Referenced during initial signature upload (ClientUploadSignature).
...
/// Return the embedded digital watermark bytes
#[inline]
pub fn watermark() -> &'static [u8] {
    WATERMARK
}

/// Size of watermark data (without random pad that gets appended at runtime)
#[inline]
pub fn watermark_len() -> usize {
    WATERMARK.len()
}

/// First 6 bytes (GIF header) "GIF89a"
pub const WATERMARK_MAGIC: [u8; 6] = [0x47, 0x49, 0x46, 0x38, 0x39, 0x61];

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn watermark_header_ok() {
        assert!(WATERMARK.len() > 100, "watermark unexpectedly short");
        assert_eq!(&WATERMARK[..6], &WATERMARK_MAGIC);
    }
}
